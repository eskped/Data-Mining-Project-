...
class Predict:
    ...
    def predict(self):
        ...
        giniDT = DecisionTreeClassifier(
            criterion='gini', max_depth=2, max_leaf_nodes=3, random_state=38)
        score = cross_validate(giniDT,
                               cv=10,  n_jobs=8, scoring=['accuracy', 'f1'], X=df, y=df_labels)
        f1 = round(score['test_f1'].mean(), 3)
        acc = round(score['test_accuracy'].mean(), 3)
        print("Decision Tree Gini")
        print("Acc and f1: "+str((acc, f1)) + "\n")

        # Gini forest
        forest = RandomForestClassifier(
            n_estimators=150, criterion='gini', max_depth=20, random_state=14)
        score = cross_validate(forest,
                               cv=10,  n_jobs=8, scoring=['accuracy', 'f1'], X=df, y=df_labels)
        f1 = round(score['test_f1'].mean(), 3)
        acc = round(score['test_accuracy'].mean(), 3)
        print("Random Forest")
